...
class XDocsReporter {
    ...
    public void generateReport(File sourceFilesDirectory, File executionRootDirectory, File issuesBy) {
                    ...
                    file(classname: bugClass) {
                        findbugsResults.BugInstance.each() { bugInstance ->
                            ...
                            if (bugInstance.Class.@classname.text() == bugClass) {
                                ...
                                def bugPath = bugInstance.SourceLine.@sourcepath[0].text()
                                log.info("BugInstance message is ${message}")

                                BugInstance(type: type, priority: priority, category: category, message: message, lineNumber: ((line) ? line : "-1"))
                                if (category == "SECURITY") {
                                    log.info("************ executionRootDirectory ${executionRootDirectory.getAbsolutePath()}")
                                    log.info("************ sourceFilesDirectory ${sourceFilesDirectory.getAbsolutePath()}")
                                    log.info("************ File Path ${bugPath}")
                                    blamer.setStartCommit(commitID)
                                    String path = "${executionRootDirectory.toURI().relativize(sourceFilesDirectory.toURI()).getPath()}${bugPath}"
                                    blamer.setFilePath(path)
                                    BlameResult blame = blamer.call()
                                    PersonIdent personIdent = blame.getSourceAuthor(Integer.parseInt(bugInstance.SourceLine.@start[0].text()))
                                    issuesBy.append(personIdent.toString() + bugInstance + "\r\n")
                                    log.info("************ personIdent ${personIdent.toString()}")
                                }
                            }
                        }
                    }
